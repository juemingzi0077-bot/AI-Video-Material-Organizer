...
def scan_videos(folder_path):
    ...
    records.sort(
        key=lambda record: record["modified_time"]
    )
...
def main():
    ...
    if not records:
        print(
            "没有找到支持的视频文件，已生成只有表头的 materials.csv。"
        )

        print(
            f"CSV 文件位置：{output_path.resolve()}"
        )
    ...
    print(
        f"整理完成：发现 {len(records)} 个视频。"
    )

    print(
        f"CSV 文件已生成：{output_path.resolve()}"
    )
